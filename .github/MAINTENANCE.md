...
- [upgrade-jsii-typescript](https://github.com/cdktf/construct-projen-template/actions/workflows/upgrade-jsii-typescript.yml): This is a custom workflow (source [here](https://github.com/cdktf/construct-projen-template/blob/main/projenrc/upgrade-jsii-typescript.ts)) that checks the [JSII support timeline](https://github.com/aws/jsii-compiler/blob/main/README.md#gear-maintenance--support) daily via [this](https://github.com/cdktf/construct-projen-template/blob/main/scripts/check-node-versions.js) script to see if the current version is less than 30 days away from EOS, and if so, creates a PR upgrading to the next supported version. The code for the upgrade itself lives in [this](https://github.com/cdktf/construct-projen-template/blob/main/scripts/update-jsii-typescript.sh) script.
